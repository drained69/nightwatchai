...
export function MarketPulse({ compact = false }) {
  ...
  return (
    <div className={compact ? 'market-pulse compact' : 'market-pulse'}>
      ...
      {macro?.dxy && <span className="mp-chip"><b>DXY</b> {macro.dxy.last.toFixed(2)} <em className={macro.dxy.changePct >= 0 ? 'down' : 'up'}>{fmtPct(macro.dxy.changePct)}</em></span>}
      {macro?.vix && <span className="mp-chip"><b>VIX</b> {macro.vix.last.toFixed(1)} <em className={macro.vix.last >= 20 ? 'down' : 'up'}>{macro.riskRegime?.replace('_', '-')}</em></span>}
